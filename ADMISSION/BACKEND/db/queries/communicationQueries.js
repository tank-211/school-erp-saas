...
export const touchTemplateLastUsedWithClient = async (
  client,
  templateId,
  schoolId
) => {
  return await prisma.message_template.updateMany({
    where: {
      id: BigInt(templateId),
      school_id: BigInt(schoolId),
    },
...
export const createCommunicationLog = async (client, payload) => {
  if (!payload?.recipient_type) {
    throw new Error(
      'recipient_type is required for communication log insertion.'
    );
  }

  if (!payload?.recipient_id) {
    throw new Error(
      'recipient_id is required for communication log insertion.'
    );
  }
  ...
  return await prisma.communication_log.create({
    data: {
      ...
      sent_at: payload.sent_at || new Date(),

...
const sendCommunicationLegacy = async (schoolId, userId, payload) => {
  ...
  const recipientId = assertPositiveInteger(
  ...
  const template = payload.template_id
    ? await communicationQueries.getTemplateById(
        assertPositiveInteger(payload.template_id, 'template_id'),
        schoolId
      )
    : null;
  ...
  const resolvedPayload = resolvePayloadFromTemplate(
    template,
    payload,
    recipient
  );
  ...
  if (!resolvedPayload.message) {
    throw new AppError(
      'message is required when template content is not available.',
      400
    );
  }
  ...
  if (payload.channel === 'email' && !resolvedPayload.subject) {
    throw new AppError(
      'subject is required for email communication.',
      400
    );
  }

  const providerResult = await dispatchCommunication(
    payload.channel,
    recipient,
    resolvedPayload
  );
  ...
  const communicationLog =
    await communicationQueries.createCommunicationLog(null, {
      school_id: schoolId,
      recipient_type: payload.recipient_type,
      recipient_id: recipientId,
  ...
  if (template) {
    await communicationQueries.touchTemplateLastUsed(
      template.id,
      schoolId
    );
  }

...
const normalizeClassValue = (value) => {
  ...
  // Support Grade 1, Grade 2, etc.
  const gradeMatch = normalized.match(/^Grade\s*(\d{1,2})$/i);
  if (gradeMatch) {
    return `Class ${gradeMatch[1]}`;
  }

  // Support Class 1, Class 2, etc. and plain numbers
...
export function MultiStepApplication() {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      ...
      const leadDesiredClass = details?.application?.lead_desired_class || "";
      ...
      setAcademicForm((prev) => ({
        ...
        marks_percentage:
          academicInfo?.marks_percentage ?? prev.marks_percentage ?? "",
        board_name:
          academicInfo?.board_name || prev.board_name || "CBSE",
        academic_year:
          academicInfo?.academic_year || prev.academic_year || "",
        additional_qualifications:
          academicInfo?.additional_qualifications ||
          prev.additional_qualifications ||
          "",
        extracurricular_activities:
          academicInfo?.extracurricular_activities ||
          prev.extracurricular_activities ||
          "",
        achievements:
          academicInfo?.achievements ||
          prev.achievements ||
          "",
      }));
    }, [details]);
    ...
    try {
      ...
      if (step === 1) {
        ...
        const payloadSchoolId =
          ...
          console.log("📚 ACADEMIC FORM BEFORE SAVE:", academicForm);
          console.log("🎯 DESIRED CLASS BEFORE SAVE:", academicForm.desired_class);
    ...
    <div className="page-sm" style={{ maxWidth: 900 }}>
      ...
      {step === 2 && (
        <ParentForm
          ...
          lead={{
            lead_first_name: details?.application?.lead_first_name || "",
            lead_last_name: details?.application?.lead_last_name || "",
            lead_email:
